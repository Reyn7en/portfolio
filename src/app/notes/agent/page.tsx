...
const chapters: Chapter[] = [
  ...
  },
  {
    ...
    sections: [
      { id: 'rt-why', title: '让 Agent 选路而不是硬编码' },
  ...
  },
  {
    ...
    sections: [
      { id: 'rl-why', title: '让 Agent 自己决定下一步' },
...
function ChapterLanggraph() {
  return <>
    ...
    <P>接触 LangGraph 后，最大的感受是：<Strong>它把 Agent 的工作流变得显式可读了。</Strong>Agent 不只是"调 LLM + 解析输出"，而是一个多步骤的工作流——每一步有明确的输入输出，步骤之间有条件跳转，中间状态需要被保存和追踪。</P>
    ...
    <NB>注意：<C>add_messages</C> 的具体行为（如同 ID 消息的更新逻辑）在不同 LangGraph 版本间可能有细微差异。</NB>
...
function ChapterState() {
  return <>
    ...
    <P>LangGraph 推荐的 State 定义方式是 <C>TypedDict</C> + <C>Annotated</C>。起初不太理解为什么要搞这么复杂，直接用 <C>dict</C> 不行吗？</P>
...
`} />
    ...
    <P>LangGraph 还提供了 <C>MessageGraph</C>，这是一个专门针对「消息列表」场景的简化版 StateGraph。两者的区别：</P>
    <Ul items={[
...
function ChapterRouterToolCalling() {
  return <>
    <H2 id="rt-why">1. 让 Agent 选路而不是硬编码</H2>
...
graph.add_conditional_edges(
    ...
    <NB>我目前只写过简单的 router_function（基于关键词匹配）。也可以让 LLM 来生成这个路由判断，这个我还没实践过，后续学到 ReAct 部分会深入。</NB>
...
function ChapterReActLoop() {
  return <>
    <H2 id="rl-why">1. 让 Agent 自己决定下一步</H2>
...
`} />
    ...
    <P>ReAct Agent 的终止条件很朴素：<Strong>LLM 不再输出 tool_calls，而是输出普通文本</Strong>。此时 <C>create_react_agent</C> 内部的循环就结束了。</P>
    <P>但有些边界情况需要小心：</P>
    <Ul items={[
    ...
    <NB>我目前没有在实际项目里遇到死循环问题。LangGraph 提供了 <C>max_iterations</C> 参数来设上限，但我还没用过。后续实践后再补充。</NB>
    ...
    <P>当然，真实场景中两者的边界经常是模糊的。比如一个看似固定的流程里夹杂了需要自主判断的步骤——这时与其硬要分类，不如直接用 ReAct 模式兜底，灵活性更高。</P>

    <H2 id="rl-summary">7. 本章回顾</H2>
    <Ul items={[
      ...
      <><Strong>和 Tool Calling Agent 的选择</Strong>取决于流程是否固定——复杂场景推荐直接用 ReAct 兜底。</>,
    ]} />
  </>
}
...
function ChapterMemorySystem() {
  return <>
    <H2 id="ms-why">1. 记忆的两个维度：会话内 vs 跨会话</H2>
    ...
    <P>Store 适合存「和用户相关但不属于当前对话 State」的信息，而 Checkpointer 适合存「当前对话的进展状态」。这里容易混淆，需要留意两者的边界。</P>
...
function ChapterHumanInTheLoop() {
  return <>
    <H2 id="hl-why">1. 什么时候必须让用户介入</H2>
    <P>LangGraph 的核心理念之一是把 Agent 的每一步都变成<Strong>显式的、可中断的、可恢复的</Strong>。这和传统 Agent 框架很不一样——很多框架把 LLM 的推理过程封装成一个黑盒，你想让人审一下再执行，往往要在 prompt 层面 hack，非常不优雅。</P>
...
def human_approval_node(state: State) -> dict:
    ...
    <NB><C>interrupt</C> 并不会抛异常或修改 State。它更像是一个「协程 yield」——把控制权交还给调用方，State 保持不变，等恢复时从 <C>interrupt</C> 调用后的下一行继续执行。</NB>
...
def resume_graph(thread_id: str, approved: bool, comment: str):
    ...
    <P><Strong>恢复时不需要从头重跑</Strong>。Checkpointer 已经把中断前的完整 State 存好了，恢复时 LangGraph 会从断点继续，之前的节点不会重复执行。这对长链路 Agent 来说非常重要——你不会想让人审批一次就把前面 10 步全部重跑一遍。</P>

    <H2 id="hl-approval-flow">4. 审批流设计模式</H2>
    <P>以下是一个基于「邮件发送 Agent」场景的 Human-in-the-Loop 案例，整理成一个可复用的设计模式。</P>
...
graph.add_conditional_edges("human_approval_node", after_approval)`} />
    ...
    <NB>一个实用的设计：如果审批被拒绝，可以让图转到一个新的「重新规划」节点，让模型根据拒绝原因重新生成方案，而不是直接 END。这样 Agent 有自我修正的机会，体验更好。</NB>
    ...
    <P>推荐的做法是：把「待执行操作」和「执行结果」分开存在 State 里。中断时展示「待执行操作」，恢复后执行，执行完把结果写回 State。这样整个流程的记录是完整的，出了问题可以回溯。</P>
...
with SqliteSaver.from_conn_string("checkpoints.db") as saver:
    ...
    <P>一个更简单的替代方案是：<Strong>不在流式过程中介入，而是在模型输出完成后、工具执行前介入</Strong>。这样实现简单很多，也能覆盖大多数场景。</P>
    ...
    <Ul items={[
      ...
      '流式输出中的实时介入实现较复杂，建议改成「输出完成后介入」更简单。',
    ]} />
    <NB>Human-in-the-Loop 的概念体系已比较清晰。如果后续实践中遇到坑，会回来补充。</NB>
  </>
}
...
function ChapterMultiAgent() {
  return <>
    <H2 id="ma-why">1. 单 Agent 的天花板在哪</H2>
    <P>在接触 Multi-Agent 架构之前，我写的 Agent 都是「单兵作战」：一个图，一个 LLM，一把工具。但当任务复杂度上升之后，单体 Agent 会遇到几个天花板：</P>
    <Ul items={[
    ...
    <NB>Harrison Chase 曾指出：「Agent 的边界应该和人类的角色边界对齐」。一个研究团队里有研究员、有工程师、有项目经理，Agent 团队也该这样分工。</NB>
...
# ...`} />
    <NB>分层不是越深越好。2-3 层通常够用，再深了调试和状态管理会变得非常复杂。目前来看 2 层（总控 + 执行）对大多数场景已经足够。</NB>
    ...
    <Ol items={[
      ...
      <><Strong>混合路由</Strong>：先走规则路由，规则匹配不上的再走 LLM 路由。推荐这种混合方式，兼顾效率和覆盖率。</>,
    ]} />
...
function ChapterRAG() {
  return <>
    ...
    <NB>RAG 的本质是「给 LLM 提供外部知识」，Agent 的本质是「让 LLM 自主决定下一步做什么」。两者结合后，LLM 可以自主决定「我还需要查更多资料」、「刚才查的不对，换个 query 再查」。</NB>
    ...
    <P>关键设计是：<Strong>Agentic RAG 里，检索是一个 Tool，不是 pipeline 的前置步骤</Strong>。这意味着 Agent 可以在任意时刻调用检索，甚至多次调用不同检索工具（向量库检索 + 网页搜索 + SQL 查询）后综合结果。</P>
...
graph.add_conditional_edges("tools", should_continue)`} />
    ...
    <NB>工具描述是检索工具设计里最值得花时间调的部分。譬如，Harrison 演示过一个例子——改了工具的 docstring 之后，Agent 调用检索工具的准确率明显提升了。</NB>
...
graph.set_entry_point("retrieve")`} />
    ...
    <P>这种写法的好处：<Strong>模型可以基于检索结果的质量动态决定下一步</Strong>，而不是靠一个固定规则。这比 Naive RAG 的准确率高不少，尤其是在需要多视角信息的复杂问题上。</P>

    <H2 id="ra-end-to-end">5. 端到端综合案例</H2>
    <P>以下是一个综合案例：研究助手 Agent。用户抛一个研究性问题（比如「对比 LangChain 和 LangGraph 在 Agent 构建上的差异」），Agent 自动完成多源检索、信息综合、答案生成。</P>
...
graph.add_edge("synthesize_answer", END)`} />
    ...
    <P>目前来看，最实用的评估方式是：<Strong>用一组固定 test case，每次改了 prompt / 工具描述 / 检索参数后，跑一遍所有 case，对比答案质量和检索步数</Strong>。不需要很复杂的评估框架，暴力但有效。</P>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-12 flex gap-10 items-start">
        ...
        <div className="flex-1 min-w-0" ref={contentRef}>
          ...
          <p className="text-slate-400 text-sm leading-relaxed max-w-xl mb-8">
            ...
            ReAct 循环、记忆系统、Human-in-the-Loop、Multi-Agent 与 RAG 集成。<br/>
            <span className="text-slate-600 text-xs">注：以下内容在 AI 辅助下整理。核心概念的理解、代码示例和实践总结经过了人工校对。部分 API 行为可能随 LangGraph 版本更新有变化，请以官网文档为准。</span>
          </p>
