...
const sections: Section[] = [
  { id: 'wsl2-intro',      chapter: '第一章',  title: 'WSL2 是什么、为什么用' },
  { id: 'wsl2-install',    chapter: '第一章',  title: 'WSL2 安装' },
  { id: 'ubuntu-config',   chapter: '第一章',  title: 'Ubuntu 基础配置' },
  { id: 'filesystem',      chapter: '第一章',  title: '文件系统互通' },
  { id: 'dev-tools',       chapter: '第一章',  title: '开发工具链安装' },
  { id: 'wsl2-issues',     chapter: '第一章',  title: '常见问题 & 踩坑' },
]

const chapters = [
  { id: 'ch1', label: '第一章', title: 'WSL2 & Ubuntu 开发环境搭建', firstSection: 'wsl2-intro' },
  { id: 'ch2', label: '第二章', title: '常用命令速查', firstSection: 'cmd-files', pending: true },
  { id: 'ch3', label: '第三章', title: 'Shell 脚本与自动化', firstSection: '', pending: true },
  { id: 'ch4', label: '第四章', title: '终端工具链配置', firstSection: '', pending: true },
  { id: 'ch5', label: '第五章', title: 'SSH 远程开发与服务器运维', firstSection: '', pending: true },
  { id: 'ch6', label: '第六章', title: '性能监控与调优', firstSection: '', pending: true },
  { id: 'ch7', label: '第七章', title: 'Docker 基础与实践', firstSection: '', pending: true },
]
...
function H2({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <h2 id={id} className="text-xl font-bold text-white mt-12 mb-4 scroll-mt-24 pb-2 border-b border-slate-800">
      {children}
    </h2>
  )
}
...
function NB({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-blue-950/40 border border-blue-800/40 rounded-lg px-4 py-3 my-4 text-sm text-blue-300 leading-relaxed">
      💡 {children}
    </div>
  )
}
...
function Warn({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-yellow-950/30 border border-yellow-700/30 rounded-lg px-4 py-3 my-4 text-sm text-yellow-300 leading-relaxed">
      ⚠️ {children}
    </div>
  )
}
...
function Ul({ items }: { items: React.ReactNode[] }) {
  return (
    <ul className="list-disc list-inside text-slate-400 text-[0.9375rem] leading-relaxed space-y-1 mb-3 ml-2">
      {items.map((item, i) => <li key={i}>{item}</li>)}
    </ul>
  )
}

function PendingChapter({ label, title }: { label: string; title: string }) {
  return (
    <div id={`pending-${label}`} className="mt-16 scroll-mt-24 opacity-50">
      <div className="border border-slate-700/50 rounded-xl p-8 text-center">
        <p className="text-slate-500 text-xs font-mono mb-2">{label}</p>
        <p className="text-slate-400 font-semibold">{title}</p>
        <p className="text-slate-600 text-sm mt-2">整理中，敬请期待…</p>
      </div>
    </div>
  )
}
        ...
        <nav className="space-y-0.5">
          {chapters.map((ch) => (
            <div key={ch.id}>
              {/* chapter header */}
              <button
                onClick={() => !ch.pending && ch.firstSection && scrollTo(ch.firstSection)}
                className={`w-full text-left px-2 py-1.5 rounded text-xs font-semibold transition-colors
                  ${ch.pending ? 'text-slate-600 cursor-default' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 cursor-pointer'}`}
              >
                <span className="font-mono text-[0.65rem] text-slate-600 mr-1.5">{ch.label}</span>
                {ch.title}
              </button>
              {/* sub-sections (only for ch1 for now) */}
              {ch.id === 'ch1' && sections.map((s) => (
                <button
                  key={s.id}
                  onClick={() => scrollTo(s.id)}
                  className={`w-full text-left px-3 py-1 rounded text-xs transition-colors pl-5
                    ${activeId === s.id
...
export default function NotesLinux() {
  ...
  return (
    <SubPageLayout title="Linux 笔记" backHref="/#notes">
      <div className="flex gap-10 items-start">
        ...
        <article className="flex-1 min-w-0">
          ...
          {/* ══ 第一章 ══════════════════════════════ */}
          <div className="mb-2">
            <p className="text-xs font-mono text-slate-600 mb-1">第一章</p>
            <h2 className="text-lg font-bold text-slate-300 mb-6">WSL2 & Ubuntu 开发环境搭建</h2>
          </div>
...
echo "deb [signed-by=/etc/apt/keyrings/example.gpg] https://example.com/ stable main" \\
  | sudo tee /etc/apt/sources.list.d/example.list`} />
          ...
          {/* ══ 待整理章节占位 ══ */}
          {chapters.filter(c => c.pending).map(ch => (
            <PendingChapter key={ch.id} label={ch.label} title={ch.title} />
          ))}
